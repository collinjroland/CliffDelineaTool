...
local_scale = 10  # How many adjacent points to consider as a local scale?
base_max_elev = 183.5  # What is the top limit for cliff base elevation (m)?
base_sea_slope = 8  # What is the max seaward slope for cliff base (deg)?
base_land_slope = 15  # What is the min landward slope for cliff base (deg)?
...
prop_convex = 0.5  # What is the minimal proportion of the distance from trendline #2 to replace modelled cliff top location?
...
params_lax = {"local_scale": local_scale, 
          "base_max_elev": base_max_elev-1.5,
          "base_sea_slope": base_sea_slope, 
...
def process_transect(data, num_transect, transects_mod):
    # Debugging
    # n = 0
    ...
    if row_count > 0:  # process transect only if there are data
        ...
        for local_point in range(local_scale, row_count - local_scale):  # Array from local_scale (10) to transect_length (90) minus local scale

            # seaward slopes
            count = 0
            for local_scale_window_point in range(1, local_scale + 1):
...
def delineate_cliff_features(data_path, number, fileName, out_bluff_top_dir,
    ...
    for num_transect in range(min(data['TransectID']), max(data['TransectID']) + 1):
        transects_mod = process_transect(data, num_transect, transects_mod)
...
pool = ThreadPool(4)
zip_obj = zip(data_path, number, fileName, out_bluff_top_dir, out_bluff_base_dir, params_strict, params_lax)
data_tuple = pool.map(delineate_cliff_features, zip_obj)

out_bluff_top_dir = Path(r'D:\CJR\LakeSuperior_BluffDelin\2009\delineation_top_points')
out_bluff_base_dir = Path(r'D:\CJR\LakeSuperior_BluffDelin\2009\delineation_base_points')
os.chdir(r"D:\CJR\LakeSuperior_BluffDelin\2009\delineation_points_text")
